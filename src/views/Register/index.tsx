        ...
        {loading && (
          ...
          >
            ...
            <h2 style={{ fontFamily: "'Outfit', sans-serif", fontSize: 28, fontWeight: 800, color: "var(--text-main)", marginBottom: 8 }}>
              Provisioning {activeRole.name} Account...
            </h2>
          ...
          {pendingVerification ? (
            <form onSubmit={handleVerifyEmail}>
              <p style={{ fontSize: 14, color: "var(--text-muted)", marginBottom: 16, lineHeight: 1.5 }}>
                We sent a verification code to <strong style={{ color: "var(--text-main)" }}>{form.email}</strong>. Enter it below to finish creating your account.
              </p>
              <div style={{ marginBottom: 16 }}>
                <label style={{ display: "block", fontSize: 11, fontWeight: 700, color: "var(--text-muted)", marginBottom: 6, fontFamily: "'Fira Code', monospace", letterSpacing: 1 }}>VERIFICATION CODE</label>
                <input
                  type="text"
                  value={verifyCode}
                  onChange={(e) => setVerifyCode(e.target.value)}
                  placeholder="123456"
                  style={{ width: "100%", padding: "12px 16px", background: "var(--bg-alt)", border: "1.5px solid var(--border-light)", borderRadius: 12, color: "var(--text-main)", fontSize: 15, outline: "none", letterSpacing: 4, fontFamily: "'Fira Code', monospace" }}
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  if (error) setErrorMessage(error.message || "Could not resend code.");
                }}
                style={{ width: "100%", marginTop: 12, padding: "10px", background: "transparent", border: "none", color: activeRole.accent, fontSize: 13, fontWeight: 700, cursor: "pointer" }}
              >
                Resend code
              </button>
            </form>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ position: "relative" }}>
                  ...
                  <button 
                    ...
                    style={{ position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", color: getEyeIconColor(form.password), cursor: "pointer", transition: "all 0.3s ease", filter: form.password ? `drop-shadow(0 0 6px ${getEyeIconColor(form.password)}88)` : "none" }}
