...
export default function OnboardingStage4() {
  ...
  const [crashOrder, setCrashOrder] = useState([]);
  const [crashKnown, setCrashKnown] = useState([]);
  const [crashAdded, setCrashAdded] = useState([]);
  ...
  const [ugOrder, setUgOrder] = useState([]);
  const [ugKnown, setUgKnown] = useState([]);
  const [ugAdded, setUgAdded] = useState([]);
        ...
        {view === "selection" && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <button
                onClick={() => navigate("/onboarding/stage3")}
                style={{
