    ...
    <section style={{ padding: "100px 32px", background: "#fcfdff" }}>
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
              ...
              <div style={{ flex: "1 1 500px" }}>
                <HoverCard style={{ height: 400, borderRadius: 24, overflow: "hidden", position: "relative" }}>
                  ...
                  <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, background: "linear-gradient(to top, rgba(26,26,46,0.95), transparent)", padding: "40px 32px 32px" }}>
                    <h3 style={{ fontFamily: "'Outfit', sans-serif", fontSize: 28, fontWeight: 700, color: "#fff", marginBottom: 8 }}>{story.name}</h3>
              ...
              <div style={{ flex: "1 1 500px" }}>
                ...
                <div style={{ display: "inline-flex", gap: 16 }}>
                  <div style={{ background: "#f0f0ff", padding: "12px 24px", borderRadius: 12 }}>
                    ...
                    <div style={{ color: "#1a1a2e", fontWeight: 600 }}>{story.before}</div>
                  </div>
                  <div style={{ background: "#e8faf5", padding: "12px 24px", borderRadius: 12 }}>
                    ...
                    <div style={{ color: "#1a1a2e", fontWeight: 600 }}>{story.after}</div>
                  </div>
                </div>
              </div>
            </motion.div>
        ...
        <div style={{ display: "flex", justifyContent: "center", alignItems: "center", gap: 24, marginTop: 48 }}>
          <button onClick={prevStory} style={{ width: 48, height: 48, borderRadius: "50%", border: "1px solid #eaecff", background: "#fff", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: "#1a1a2e", transition: "all 0.2s" }} onMouseEnter={e => e.currentTarget.style.background = "#f0f4ff"} onMouseLeave={e => e.currentTarget.style.background = "#fff"}>
            <ArrowLeft size={20} />
          </button>
          
          <div style={{ display: "flex", gap: 8 }}>
            {stories.map((_, i) => (
              <button key={i} onClick={() => setCurrentIndex(i)} style={{ width: 10, height: 10, borderRadius: "50%", background: i === currentIndex ? "#6c63ff" : "#eaecff", border: "none", cursor: "pointer", transition: "all 0.3s", transform: i === currentIndex ? "scale(1.2)" : "scale(1)" }} aria-label={`Go to slide ${i + 1}`} />
            ))}
          </div>

          <button onClick={nextStory} style={{ width: 48, height: 48, borderRadius: "50%", border: "1px solid #eaecff", background: "#fff", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: "#1a1a2e", transition: "all 0.2s" }} onMouseEnter={e => e.currentTarget.style.background = "#f0f4ff"} onMouseLeave={e => e.currentTarget.style.background = "#fff"}>
            <ArrowRight size={20} />
          </button>
        </div>
