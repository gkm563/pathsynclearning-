...
export default function PlatformChallenges() {
  ...
  const [challenges, setChallenges] = useState(() => {
    try {
      ...
      console.error("Error reading saved challenges state:", e);
    }
  ...
  const [proIdeChallenge, setProIdeChallenge] = useState(null);
  const [proAssessmentActive, setProAssessmentActive] = useState(null); // Dedicated Fullscreen Assessment Page
    ...
    <DashboardLayout activeTab={activeTab} setActiveTab={handleTabChange}>
      <div style={{ display: "flex", flexDirection: "column", gap: 24, paddingBottom: 60 }}>
        ...
        <div style={{
          ...
          <button
            onClick={() => navigate("/dashboard?tab=memory-lane")}
            style={{
...
function ProAssessmentSubpage({ challenge, onClose, onSubmit }) {
  ...
  const [userReasonings, setUserReasonings] = useState({}); // { [qId]: text }
